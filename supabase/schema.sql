...
create policy "public reads website pages" on public.public_website_pages for select using (true);
create policy "members create website pages" on public.public_website_pages for insert with check (public.is_tenant_member(tenant_id));
create policy "members update website pages" on public.public_website_pages for update using (public.is_tenant_member(tenant_id)) with check (public.is_tenant_member(tenant_id));
create policy "members delete website pages" on public.public_website_pages for delete using (public.is_tenant_member(tenant_id));
create index if not exists public_website_pages_tenant_path_idx on public.public_website_pages (tenant_id, path);
