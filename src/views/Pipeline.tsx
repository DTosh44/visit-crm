...
const stages: PipelineStage[] = ['New lead', 'Qualified', 'Proposal', 'Decision', 'Won', 'Lost']
const stageColours: Record<PipelineStage, string> = { 'New lead': '#6b7788', Qualified: '#3773b9', Proposal: '#6858ce', Decision: '#d28d30', Won: '#278362', Lost:'#9b5260' }
const isClosed=(stage:PipelineStage)=>stage==='Won'||stage==='Lost'
...
export function Pipeline({createRequest=0}:{createRequest?:number}) {
  ...
  const [draft,setDraft]=useState({organisationName:'',contactName:'',stage:'New lead' as PipelineStage,proposedLevel:data.levels[0]?.name??'',value:data.levels[0]?.price??0,probability:15,source:'Website enquiry',nextAction:'Arrange discovery call',nextActionDate:new Date().toISOString().slice(0,10),owner:'Morgan Lee',outcomeDate:'',lostReason:''})
  const edit=(item:Opportunity)=>{setDraft({organisationName:item.organisationName,contactName:item.contactName,stage:item.stage,proposedLevel:item.proposedLevel,value:item.value,probability:item.probability,source:item.source,nextAction:item.nextAction,nextActionDate:item.nextActionDate,owner:item.owner,outcomeDate:item.outcomeDate??'',lostReason:item.lostReason??''});setEditingId(item.id);setAdding(true)}
  ...
  const totalValue = opportunities.filter((item) => !isClosed(item.stage)).reduce((sum, item) => sum + item.value, 0)
  const weightedValue = opportunities.filter((item) => !isClosed(item.stage)).reduce((sum, item) => sum + item.value * item.probability / 100, 0)
  ...
  return (
    <div className="pipeline-page">
      ...
      <section className="pipeline-summary">
        ...
        <div><p><small>Live opportunities</small><strong>{opportunities.filter((item) => !isClosed(item.stage)).length}</strong></p></div>
