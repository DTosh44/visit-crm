...
import { Badge, Button, EmptyState, PageHeader, useDialogAccessibility } from '../components/UI'
...
export function Events() {
  ...
  return <div>
    ...
    <section className="panel data-panel events-panel">
      <div className="table-toolbar"><div className="table-search"><Search size={17}/><input type="search" aria-label="Search events venues or organisers" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search events, venues or organisers..."/></div><label className="select-wrap"><select aria-label="Filter events by status" value={status} onChange={(e)=>setStatus(e.target.value)}><option>All</option><option>Published</option><option>Draft</option><option>In review</option><option>Changes requested</option><option>Withdrawn</option></select></label></div>
      {filtered.length ? <div className="table-scroll" tabIndex={0} aria-label="Events table. Scroll to view all columns."><table className="data-table events-table"><thead><tr><th>Event</th><th>Date</th><th>Venue</th><th>Submitted by</th><th>Status</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{filtered.map((event)=><tr key={event.id}>
