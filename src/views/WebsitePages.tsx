...
export function WebsitePages({createRequest=0}:{createRequest?:number}) {
  ...
  const run = async (action: () => Promise<unknown>) => { setBusy(true); setActionError(''); try { await action() } catch (error) { setActionError(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) } }
  ...
  return <div>
    ...
    <section className="panel data-panel">
      ...
      <div className="table-scroll"><table className="data-table cms-page-table"><thead><tr><th>Page</th><th>Route</th><th>Template</th><th>Updated</th><th>Status</th><th>Actions</th></tr></thead><tbody>{pages.map((page) => <tr key={page.id}><td><div className="org-cell"><FileText size={18}/><div><strong>{page.name}</strong><small>{page.draft.title}</small></div></div></td><td><code>{page.path}</code></td><td>{page.template}</td><td>{page.updatedAt}</td><td><Badge>{page.status}</Badge>{page.published && <small className="published-version">Live v{page.version}</small>}</td><td><div className="event-row-actions">{page.status !== 'Published' && <Button size="sm" icon={Send} disabled={busy} onClick={() => void run(() => publishWebsitePage(page.id))}>Publish</Button>}<button className="icon-button" onClick={() => preview(page)} title="Preview draft" aria-label={`Preview ${page.name}`}><Eye size={16}/></button>{page.status === 'Draft changes' && <button className="icon-button" disabled={busy} onClick={() => { if(confirm(`Discard draft changes to ${page.name}?`)) void run(() => discardWebsitePageDraft(page.id)) }} title="Discard draft" aria-label={`Discard draft changes to ${page.name}`}><RotateCcw size={16}/></button>}<button className="icon-button" onClick={() => void copyBrief(page)} title="Copy AI editing prompt" aria-label={`Copy AI editing prompt for ${page.name}`}><Sparkles size={16}/></button><button className="icon-button" onClick={() => edit(page)} title="Edit page" aria-label={`Edit ${page.name}`}><Edit3 size={16}/></button>{page.template === 'Landing page' && <button className="icon-button danger" disabled={busy} onClick={() => { if(confirm(`Delete ${page.name}?`)) void run(() => deleteWebsitePage(page.id)) }} title="Delete page" aria-label={`Delete ${page.name}`}><Trash2 size={16}/></button>}</div></td></tr>)}</tbody></table></div>
    </section>
    {editing && <WebsitePageEditor page={editing} onClose={() => setEditing(null)} onSave={async (draft) => { await updateWebsitePageDraft(editing.id, draft); setEditing(null) }} onRestore={async (revisionId) => { await restoreWebsitePageVersion(editing.id, revisionId); setEditing(null) }} />}
    {creating && <NewPageModal existingPaths={data.websitePages.map((page) => page.path)} onClose={() => setCreating(false)} onCreate={async (name, path, content) => { const page = await createWebsitePage({ name, path, content }); setCreating(false); edit(page) }} />}
  </div>
}

function WebsitePageEditor({ page, onClose, onSave, onRestore }: { page: WebsitePage; onClose: () => void; onSave: (draft: WebsitePageContent) => Promise<void>; onRestore: (revisionId: number) => Promise<void> }) {
  const [draft, setDraft] = useState<WebsitePageContent>(structuredClone(page.draft))
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
  const submit = async (action: () => Promise<void>) => { setBusy(true); setError(''); try { await action() } catch (failure) { setError(failure instanceof Error ? failure.message : String(failure)) } finally { setBusy(false) } }
  ...
  return <Modal title={`Edit ${page.name}`} subtitle={`${page.path} · Changes stay private until published.`} width="lg" onClose={onClose}>
    <form className="form-stack" onSubmit={(event) => { event.preventDefault(); void submit(() => onSave(draft)) }}>
      ...
      {page.revisions && <PageRevisionHistory revisions={page.revisions} busy={busy} onRestore={(revisionId) => { if(confirm('Restore this published version as a new draft?')) void submit(() => onRestore(revisionId)) }} />}
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="modal-actions"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit" disabled={busy}>Save draft</Button></div>
    </form>
  </Modal>
}

function NewPageModal({ existingPaths, onClose, onCreate }: { existingPaths: string[]; onClose: () => void; onCreate: (name: string, path: string, content: WebsitePageContent) => Promise<void> }) {
  const [name, setName] = useState(''); const [path, setPath] = useState('/'); const [error, setError] = useState(''); const [busy,setBusy]=useState(false)
  const submit = async (event: FormEvent) => { event.preventDefault(); const normalized = `/${path.replace(/^\/+|\/+$/g, '')}`; if (existingPaths.includes(normalized)) { setError('That website route is already managed.'); return } const draft = blankContent(); draft.title = name; draft.metaTitle = name; draft.navigationLabel = name; setBusy(true); try { await onCreate(name, normalized, draft) } catch(failure) { setError(failure instanceof Error ? failure.message : String(failure)); setBusy(false) } }
  return <Modal title="New landing page" subtitle="Create a structured route that can be edited manually or through an AI change prompt." onClose={onClose}><form className="form-stack" onSubmit={(event)=>{void submit(event)}}><Field label="Internal page name"><input autoFocus required value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Website route"><input required value={path} onChange={(event) => setPath(event.target.value)} placeholder="/campaign" /></Field>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit" disabled={busy}>Create draft</Button></div></form></Modal>
}
