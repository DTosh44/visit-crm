...
export function Organisations({ onAdd, onOpen }: { onAdd: () => void; onOpen: (organisation: Organisation) => void }) {
  ...
  return (
    <div>
            ...
            <table className="data-table organisations-table">
              ...
              <tbody>{organisations.map((org) => {
                ...
                return <tr key={org.id}>
                  ...
                  <td><button className="icon-button danger" onClick={(event) => {event.stopPropagation();if(data.listings.some((item)=>item.organisationId===org.id)||data.events.some((item)=>item.organisationId===org.id)){window.alert('Move or remove linked listings and events before deleting this organisation.');return}if(window.confirm(`Delete ${org.name} and its linked records?`))deleteOrganisation(org.id)}} aria-label={`Delete ${org.name}`}><Trash2 size={17} /></button></td>
                </tr>
              })}</tbody>
            </table>
          </div>
      ...
      {mergeOpen&&<Modal title="Review suspected duplicates" subtitle="Nothing is merged automatically. Select the surviving record; linked CRM and specialist records will be relinked and the action audited." onClose={()=>setMergeOpen(false)}>{mergeError&&<p role="alert" className="form-error">{mergeError}</p>}{duplicateGroups.length?<div className="form-stack">{duplicateGroups.map(([key,items])=><section className="panel" key={key}><h3>{items[0].name} · {items[0].town}</h3><p>Choose the record whose field values should survive. Contacts, memberships, invoices, agreements, listings, tasks and specialist references from the other record are retained and relinked.</p>{items.map((item)=><label className="choice-list" key={item.id}><input type="radio" name={key} checked={(mergeSelection[key]??items[0].id)===item.id} onChange={()=>setMergeSelection({...mergeSelection,[key]:item.id})}/><span><strong>{item.name}</strong> · {item.status} · {item.tier} · owner {item.owner||'unknown'} · {item.id}</span></label>)}<Button variant="secondary" onClick={()=>{void (async()=>{setMergeError('');try{const survivorId=mergeSelection[key]??items[0].id;for(const duplicate of items.filter((item)=>item.id!==survivorId)){await mergeOrganisations(survivorId,duplicate.id);replaceOrganisationReferences(duplicate.id,survivorId)}}catch(error){setMergeError(error instanceof Error?error.message:'Organisation merge failed')}})()}}>Merge reviewed records</Button></section>)}</div>:<EmptyState icon={Merge} title="No suspected duplicates" description="Exact name and town matches will appear here for review. Similar names are never merged automatically."/>}<div className="modal-actions"><Button onClick={()=>setMergeOpen(false)}>Done</Button></div></Modal>}
    </div>
  )
}
