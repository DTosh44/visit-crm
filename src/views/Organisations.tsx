        ...
        {organisations.length ? (
          <div className="table-scroll organisation-table-scroll" tabIndex={0} aria-label="Organisation results. Scroll to view all matching organisations.">
            <table className="data-table organisations-table">
              <thead><tr><th className="checkbox-cell"><input type="checkbox" aria-label="Select all matching organisations" checked={selected.length === organisations.length && organisations.length > 0} onChange={toggleAll} /></th><th>Organisation</th><th>Membership</th><th>Health</th><th>Location</th><th>Renewal</th><th>Value</th><th>Owner</th><th><span className="sr-only">Actions</span></th></tr></thead>
              <tbody>{organisations.map((org) => {
                ...
                return <tr key={org.id}>
                  ...
                  <td><div className="org-cell"><Avatar name={org.name} colour={org.colour} /><div><button type="button" className="table-row-link" onClick={() => onOpen(org)}>{org.name}</button><small>{contact?.name ?? 'No primary contact'} · {org.type}</small></div></div></td>
