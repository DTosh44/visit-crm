...
export function Organisations({ onAdd, onOpen }: { onAdd: () => void; onOpen: (organisation: Organisation) => void }) {
  ...
  const importCsv=async(file?:File)=>{if(!file)return;const rows=parseCsv(await file.text());const headers=rows.shift()?.map((value)=>value.trim().toLowerCase())??[];if(!headers.includes('name')){window.alert('The CSV must contain a name column. No records were imported.');return}let imported=0,skipped=0;for(const values of rows){const row=Object.fromEntries(headers.map((header,index)=>[header,values[index]?.trim()??'']));if(!row.name||data.organisations.some((org)=>org.name.toLowerCase()===row.name.toLowerCase()&&org.town.toLowerCase()===(row.town||'Valechester').toLowerCase())){skipped++;continue}const status=(['Active','Renewing','Prospect','Free listing','Lapsed','Non-member'].includes(row.status)?row.status:'Non-member') as Organisation['status'];addOrganisation({name:row.name,type:row.type||'Attractions',town:row.town||'Valechester',contactName:row.contactname||row['contact name']||'Primary contact',contactEmail:row.contactemail||row['contact email']||'',tier:status==='Non-member'?'No membership':data.levels.some((level)=>level.active&&level.name===row.tier)?row.tier:data.levels.find((level)=>level.active)?.name??'Free Listing',status,nextAction:row.nextaction||row['next action']||'Review imported record'});imported++}window.alert(`${imported} organisation${imported===1?'':'s'} imported${skipped?`; ${skipped} duplicate or incomplete row${skipped===1?'':'s'} skipped`:'.'}`)}

  return (
    <div>
      <PageHeader eyebrow="CRM" title="Organisations" description="Manage members, prospects, contacts and every relationship in one place." actions={<><label className="button button-secondary button-md"><Upload size={17}/>Import CSV<input hidden type="file" accept=".csv,text/csv" onChange={(e)=>void importCsv(e.target.files?.[0])}/></label><Button variant="secondary" icon={Merge} onClick={()=>{const count=deduplicateOrganisations();window.alert(count?`${count} duplicate organisation${count===1?'':'s'} removed.`:'No duplicates found.')}}>Deduplicate</Button><Button variant="secondary" icon={Download} onClick={()=>downloadCsv('organisations.csv',[['Organisation','Type','Town','Relationship','Status','Owner','Renewal','Annual value'],...organisations.map((org)=>[org.name,org.type,org.town,org.tier,org.status,org.owner,org.renewalDate,org.annualValue])])}>Export</Button><Button icon={Plus} onClick={onAdd}>Add organisation</Button></>} />
