...
import { downloadCsv, escapeHtml, openEmail, printHtml } from '../actions'

export function Billing({ onCreate }: { onCreate: () => void }) {
  const { data, markInvoicePaid, toggleInvoiceReminders, updateInvoice, deleteInvoice } = useCRM()
  const {features}=useFeatures()
  const remindersReady=features.communications&&features.automations
  ...
  return (
    <div>
      ...
      <section className="billing-stats">
        ...
        {remindersReady&&<article><span className="stat-icon amber"><BellOff size={18} /></span><div><p>Reminders paused</p><h3>{data.invoices.filter((item) => item.remindersPaused).length}</h3><small>Review payment arrangements</small></div></article>}
      </section>

      <section className="panel data-panel">
        <div className="table-toolbar"><div className="table-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search invoices" placeholder="Search invoices..." /></div><div className="toolbar-filters"><label className="select-wrap"><select aria-label="Filter invoices by status" value={status} onChange={(event) => setStatus(event.target.value)}><option>All statuses</option><option>Draft</option><option>Sent</option><option>Overdue</option><option>Paid</option><option>Void</option></select><ChevronDown size={14} /></label></div></div>
        <div className="table-scroll"><table className="data-table invoice-table"><thead><tr><th>Invoice</th><th>Organisation</th><th>Issued</th><th>Due</th><th>Total</th><th>Status</th><th>{remindersReady?'Reminders':'Follow-up'}</th><th /></tr></thead><tbody>{invoices.map((invoice) => {
          const org = data.organisations.find((item) => item.id === invoice.organisationId)
          return <tr key={invoice.id}><td><div className="invoice-number"><button className="table-primary-action" onClick={() => setSelectedInvoice(invoice)}>{invoice.number}</button><small>{invoice.description}</small></div></td><td><div className="org-cell"><Avatar name={org?.name ?? ''} colour={org?.colour} size="sm" /><strong>{org?.name}</strong></div></td><td>{formatDate(invoice.issueDate, { day: 'numeric', month: 'short' })}</td><td><span className={invoice.status === 'Overdue' ? 'date-overdue' : ''}>{formatDate(invoice.dueDate, { day: 'numeric', month: 'short' })}</span></td><td><strong>{currency.format(invoice.total)}</strong></td><td><Badge>{invoice.status}</Badge></td><td>{invoice.status === 'Paid' || invoice.status === 'Draft' ? <span className="muted">—</span> : remindersReady ? invoice.remindersPaused ? <span className="reminder-state paused"><PauseCircle size={14} />Paused</span> : <span className="reminder-state active"><PlayCircle size={14} />Active</span> : <span className="muted">Manual</span>}</td><td><button className="icon-button" aria-label={`Open actions for ${invoice.number}`} onClick={(event) => {event.stopPropagation();setSelectedInvoice(invoice)}}><MoreHorizontal size={17} /></button></td></tr>
        })}</tbody></table></div>
      ...
      {selectedInvoice && (() => {
        ...
        return <Drawer title={liveInvoice.number} subtitle={liveInvoice.description} onClose={() => setSelectedInvoice(null)} width="standard">
          ...
          <div className="invoice-sheet">
            <header><div className="invoice-brand"><span><CircleDollarSign size={19} /></span><strong>{data.workspace.destinationName}</strong></div><div><small>Invoice number</small><strong>{liveInvoice.number}</strong></div></header>
            ...
            <table><thead><tr><th>Description</th><th>Net</th></tr></thead><tbody><tr><td>{liveInvoice.description}</td><td>{currency.format(liveInvoice.subtotal)}</td></tr></tbody><tfoot><tr><td>Subtotal</td><td>{currency.format(liveInvoice.subtotal)}</td></tr><tr><td>VAT ({data.workspace.vatRate??20}%)</td><td>{currency.format(liveInvoice.vat)}</td></tr><tr><td>Total</td><td>{currency.format(liveInvoice.total)}</td></tr></tfoot></table>
          </div>
          {remindersReady?<section className="subpanel reminder-panel"><header><div><h3>Automatic reminders</h3><p>{liveInvoice.remindersPaused ? 'No reminders will be sent while paused.' : 'Next reminder is checked against payment status before sending.'}</p></div><button className={`switch ${!liveInvoice.remindersPaused ? 'on' : ''}`} role="switch" aria-checked={!liveInvoice.remindersPaused} aria-label={`Automatic reminders for ${liveInvoice.number}`} onClick={() => toggleInvoiceReminders(liveInvoice.id)}><span /></button></header><div className="reminder-steps"><span className={liveInvoice.reminderStep >= 1 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 1 ? <Check size={12} /> : '1'}</i>7 days</span><span className={liveInvoice.reminderStep >= 2 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 2 ? <Check size={12} /> : '2'}</i>14 days</span><span className={liveInvoice.reminderStep >= 3 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 3 ? <Check size={12} /> : '3'}</i>28 days</span></div></section>:<section className="subpanel"><header><div><h3>Manual follow-up</h3><p>Automatic invoice reminders are not active. Use your email app to follow up with the customer.</p></div></header></section>}
          ...
          <div className="drawer-button-stack">{liveInvoice.status === 'Draft' && <><Button icon={Mail} onClick={() => openEmail(liveInvoice.sentTo,`Invoice ${liveInvoice.number}`,`Please find invoice ${liveInvoice.number} for ${currency.format(liveInvoice.total)}. It is due on ${formatDate(liveInvoice.dueDate)}.`)}>Compose invoice email</Button><Button variant="secondary" icon={Send} onClick={()=>{if(confirm(`Mark ${liveInvoice.number} as sent? Confirm only after you have sent the invoice and attachment from your email app.`))updateInvoice(liveInvoice.id,{status:'Sent'})}}>Mark as sent</Button></>}<Button variant="secondary" icon={Mail} onClick={()=>openEmail(liveInvoice.sentTo,liveInvoice.number)}>Email customer</Button><Button variant="ghost" icon={Download} onClick={()=>printHtml(liveInvoice.number,`<h1>Invoice ${escapeHtml(liveInvoice.number)}</h1><p><strong>Billed to:</strong> ${escapeHtml(org?.name)}</p><p>${escapeHtml(liveInvoice.description)}</p><table><tr><th>Net</th><td>${escapeHtml(currency.format(liveInvoice.subtotal))}</td></tr><tr><th>VAT</th><td>${escapeHtml(currency.format(liveInvoice.vat))}</td></tr><tr class="total"><th>Total</th><td>${escapeHtml(currency.format(liveInvoice.total))}</td></tr></table><p>Due ${escapeHtml(formatDate(liveInvoice.dueDate))}</p>`)}>Print / save PDF</Button>{!['Paid','Void'].includes(liveInvoice.status)&&<Button variant="secondary" icon={XCircle} onClick={()=>updateInvoice(liveInvoice.id,{status:'Void',remindersPaused:true})}>Void invoice</Button>}<Button variant="danger" icon={Trash2} onClick={()=>{if(confirm(`Delete ${liveInvoice.number}?`)){deleteInvoice(liveInvoice.id);setSelectedInvoice(null)}}}>Delete invoice</Button></div>
        </Drawer>
      })()}
    </div>
  )
