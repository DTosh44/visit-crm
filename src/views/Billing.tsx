    ...
    <div>
      ...
      {selectedInvoice && (() => {
        ...
        return <Drawer title={liveInvoice.number} subtitle={liveInvoice.description} onClose={() => setSelectedInvoice(null)} width="standard">
          ...
          <section className="subpanel reminder-panel"><header><div><h3>Automatic reminders</h3><p>{liveInvoice.remindersPaused ? 'No reminders will be sent while paused.' : 'Next reminder is checked against payment status before sending.'}</p></div><button type="button" role="switch" aria-checked={!liveInvoice.remindersPaused} aria-label="Automatic invoice reminders" className={`switch ${!liveInvoice.remindersPaused ? 'on' : ''}`} onClick={() => toggleInvoiceReminders(liveInvoice.id)}><span aria-hidden="true" /></button></header><div className="reminder-steps"><span className={liveInvoice.reminderStep >= 1 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 1 ? <Check size={12} /> : '1'}</i>7 days</span><span className={liveInvoice.reminderStep >= 2 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 2 ? <Check size={12} /> : '2'}</i>14 days</span><span className={liveInvoice.reminderStep >= 3 ? 'done' : ''}><i>{liveInvoice.reminderStep >= 3 ? <Check size={12} /> : '3'}</i>28 days</span></div></section>
