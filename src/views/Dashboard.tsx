...
export function Dashboard({ navigate, openOrganisation }: { navigate: (view: ViewKey) => void; openOrganisation: (org: Organisation) => void }) {
  ...
  const metric=(id:WidgetId)=>{
    const map: Record<WidgetId,{value:string;detail:string;icon:typeof Building2;tone:string}> = {
      'active-members':{value:'99',detail:'+4.2% year on year',icon:Building2,tone:'purple'},
  ...
  const panel=(id:WidgetId)=>{
    if(id==='member-tiers') return <article className="dashboard-widget panel wide-widget"><div className="widget-heading"><div><h3>Members by tier</h3><p>Paying membership mix</p></div><GripVertical size={17}/></div><div className="tier-counts compact">{['Tier 1','Tier 2','Tier 3','Tier 4'].map((name)=><button key={name} onClick={()=>navigate('memberships')}><i/><span><small>{name}</small><strong>{data.levels.find((l)=>l.name===name)?.members??0}</strong><em>members</em></span></button>)}</div></article>
  ...
  return <div className="dashboard-page configurable-dashboard">
    ...
    <section className="attention-strip"><div className="attention-title"><span><CircleAlert size={18}/></span><div><strong>Today’s focus</strong><small>{openTasks.length} open actions across the team</small></div></div><button onClick={()=>navigate('billing')}><strong>{overdueInvoices.length}</strong><span>overdue invoices</span><ArrowRight size={15}/></button><button onClick={()=>navigate('memberships')}><strong>{renewals.length}</strong><span>renewals approaching</span><ArrowRight size={15}/></button><button onClick={()=>navigate('listings')}><strong>{data.listings.filter((l)=>l.status==='In review'||l.status==='Changes requested').length}</strong><span>content reviews</span><ArrowRight size={15}/></button></section>
