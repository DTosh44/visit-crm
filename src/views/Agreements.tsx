import { CheckCircle2, Clock3, Download, Eye, FileSignature, Mail, Plus, Search, Send, ShieldCheck, Trash2 } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <section className="panel data-panel">
        <div className="table-toolbar"><div className="table-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search agreements" placeholder="Search agreements..." /></div><label className="select-wrap"><select aria-label="Filter agreements by status" value={status} onChange={(e)=>setStatus(e.target.value)}><option>All statuses</option>{['Draft','Sent','Viewed','Signed','Declined','Expired'].map((item)=><option key={item}>{item}</option>)}</select></label></div>
