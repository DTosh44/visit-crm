...
export type PortalOperation='read'|'propose'|'save_event_draft'|'submit_event'|'manage_contact'|'set_primary'|'express_interest'|'apply'|'confirm'
...
export function canPortalAccess(grant:PortalGrant|undefined,target:PortalTarget,operation:PortalOperation){
  ...
  if(['express_interest','apply','confirm'].includes(operation)){
    if(target.entity!=='opportunity'||target.status!=='Open'||!target.eligibleTiers?.includes(target.tier??''))return false
