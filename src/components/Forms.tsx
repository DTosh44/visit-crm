...
export function CreateInvoiceModal({ onClose }: { onClose: () => void }) {
  ...
  const [draft, setDraft] = useState<InvoiceDraft>({ organisationId: initialOrganisation?.id ?? '', description: `${initialOrganisation?.tier ?? ''} membership 2026/27`, subtotal: initialOrganisation?.annualValue ?? 0, dueDate: '2026-10-20', sendNow: false })
  ...
  return <Modal title="Create invoice" subtitle="Raise a draft or send it to the organisation’s nominated billing contact." onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      <Field label="Organisation"><select required value={draft.organisationId} onChange={(event) => { const selected = data.organisations.find((item) => item.id === event.target.value); setDraft({ ...draft, organisationId: event.target.value, subtotal: selected?.annualValue ?? 0, description: `${selected?.tier ?? ''} membership 2026/27` }) }}>{data.organisations.filter((item) => item.status !== 'Prospect' && item.annualValue > 0).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
...
export function AddTaskModal({ onClose }: { onClose: () => void }) {
  ...
  const [draft, setDraft] = useState<TaskDraft>({ title: '', organisationId: '', dueDate: '2026-09-21', priority: 'Medium', category: 'Follow-up' })
