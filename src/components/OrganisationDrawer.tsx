  ...
  return (
    <Drawer title={organisation.name} subtitle={`${organisation.type} · ${organisation.town}`} onClose={onClose}>
      ...
      {tab === 'Listings' && <div className="org-tab-content">
        <div className="section-heading"><div><h3>Website listings</h3><p>Edit and publish this organisation’s public content from its CRM record.</p></div><Button icon={Plus} size="sm" disabled={creatingListing} onClick={()=>{const name=window.prompt('Listing name',organisation.name);if(!name)return;setCreatingListing(true);setListingError('');void createListing(organisation.id,name).then(onEditListing).catch((error:unknown)=>setListingError(error instanceof Error?error.message:'Listing could not be created')).finally(()=>setCreatingListing(false))}}>{creatingListing?'Creating…':'Add listing'}</Button></div>
        {listingError&&<p className="form-error" role="alert">{listingError}</p>}
