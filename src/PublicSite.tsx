...
function EventsPage({ savedCount }: { savedCount: number }) {
  ...
  const published=useMemo(()=>recurringEvents(data.events.filter((event)=>event.status==='Published')).sort((a,b)=>a.startDate.localeCompare(b.startDate)),[data.events])
...
function SubmitEventPage({ savedCount }: { savedCount: number }) {
  const {account,loading,signOut}=useEventAccount(); const {data,createEvent}=useCRM(); const [sent,setSent]=useState(false)
...
export function PublicSite() {
  ...
  else if (features.itineraries&&contentTypeByPath[parts[0]]&&parts[1]) {const content=data.contentPages.find((item)=>item.type===contentTypeByPath[parts[0]]&&item.slug===parts[1]&&item.status==='Published');page=content?<ContentDetailPage page={content} savedCount={savedIds.length}/>:<NotFoundPage savedCount={savedIds.length}/>}
