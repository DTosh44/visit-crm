...
import type { DestinationEvent, EventDraft, Listing } from './types'
...
function EventsPage({ savedCount }: { savedCount: number }) {
  ...
  const eventCategories=['All',...Array.from(new Set(published.map((event)=>event.category)))]
  const filtered=published.filter((event)=>(category==='All'||event.category===category)&&(!query.trim()||[event.title,event.category,event.venueName,event.town,event.description].join(' ').toLowerCase().includes(query.toLowerCase())))
  ...
  return <PublicShell savedCount={savedCount}><main><PageIntro eyebrow="What’s on" title="Make a date of Valechester." description="Markets, live performance, family evenings and the kind of local events worth building a trip around." image={imageLibrary.restaurant} /><section className="event-directory-tools site-container"><div className="event-search"><Search size={18}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search events, venues or places" aria-label="Search events"/></div><div className="event-category-filter">{eventCategories.map((item)=><button key={item} className={category===item?'active':''} onClick={()=>setCategory(item)}>{item}</button>)}</div><div className="event-directory-meta"><strong>{filtered.length} events</strong><button onClick={()=>siteNavigate('/submit-event')}>Add your event <ArrowRight size={14}/></button></div></section><section className="event-calendar site-container">{filtered.map((event) => <article id={event.id} key={event.id}><img src={imageLibrary[event.image]??imageLibrary.theatre} alt="" /><div className="event-calendar-date"><strong>{eventDay(event)}</strong><span>{eventMonth(event)}</span></div><div><span className="site-eyebrow plum">{event.category}</span><h2>{event.title}</h2><p>{event.description}</p><dl><div><dt>Where</dt><dd>{event.venueName}, {event.town}</dd></div><div><dt>When</dt><dd>{eventDate(event).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}, {eventTime(event)}</dd></div><div><dt>Tickets</dt><dd>{event.price}</dd></div></dl><div className="public-event-actions"><button onClick={() => siteNavigate(`/plan?event=${event.id}`)}>Plan a trip around this <ArrowRight size={15} /></button>{event.bookingUrl&&<a href={event.bookingUrl}>Book tickets</a>}</div></div></article>)}</section>{!filtered.length&&<div className="site-no-results site-container"><Search size={25}/><h3>No matching events</h3><p>Try a different category or search phrase.</p></div>}<NewsletterSignup /></main></PublicShell>
}
