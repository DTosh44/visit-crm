...
const listingTemplateProfiles: ListingTemplateProfile[] = [
  ...
  { tier:'Free Listing', packageName:'Free Listing', listingId:'list-011', templateName:'Branded holding page', purpose:'No business page functionality or promotional content.', accent:'#7b8798', media:'Visit Valechester logo only', includes:['Visit Valechester logo'], goodForLimit:0, imageCount:0, videoCount:0, showAtGlance:false, showReviews:false, showPlanningSummary:false, showMap:false, showAwards:false, showRelated:false, action:'website' },
]

function ListingTemplatesPage({ listings, actions }: { listings: Listing[]; actions: VisitorActions }) {
  return <PublicShell savedCount={actions.savedIds.length}><main><PageIntro eyebrow="For review" title="Member listing templates." description="Six proposed page standards, each matched to the visibility and content allowance of its membership level. Review the live examples before the format is applied to other listings." image={imageLibrary.castle}/><section className="template-review site-container"><header><span className="site-eyebrow plum">Approval set</span><h2>One working example for every membership type</h2><p>Paid templates share accurate visitor information, useful search taxonomy and clear calls to action. Higher tiers add richer storytelling, media, review insight and conversion features. The Free Listing displays the Visit Valechester logo only.</p></header><div className="template-review-grid">{listingTemplateProfiles.map((profile)=>{const listing=listings.find((item)=>item.id===profile.listingId);return <article key={profile.packageName} style={{'--template-accent':profile.accent} as CSSProperties}><div className="template-review-tier"><span>{profile.packageName}</span><small>{profile.media}</small></div><h3>{profile.templateName}</h3><p>{profile.purpose}</p><h4>Template includes</h4><ul>{profile.includes.map((item)=><li key={item}><Check size={14}/>{item}</li>)}</ul>{listing&&<button onClick={()=>siteNavigate(`/place/${listing.id}`)}>Review {listing.name} <ArrowRight size={15}/></button>}</article>})}</div><aside><strong>Approval scope</strong><p>These six example pages are the only listings using the proposed package templates. Approval will provide the basis for updating the remaining member listings and the corresponding CRM content fields.</p></aside><aside className="template-review-note"><strong>Review integration</strong><p>The design supports an authorised Google Business Profile connection or a member-supplied Tripadvisor widget. Public Google Places review requests require a billing-enabled account, so the prototype uses stored feedback themes until an approved source is connected.</p></aside></section></main></PublicShell>
}
...
function TemplateListingPage({ listing, actions, tier, profile }: { listing: Listing; actions: VisitorActions; tier: string; profile: ListingTemplateProfile }) {
  ...
  const saved = actions.savedIds.includes(listing.id)
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); recordSubmission('vv-listing-enquiries',{ listingId: listing.id, listingName: listing.name, ...values }); setSent(true) }
  if (profile.tier === 'Free Listing') return <PublicShell savedCount={actions.savedIds.length}><main className="free-listing-brand" aria-label="Visit Valechester"><BrandLogo /></main></PublicShell>
    ...
    <main className={`place-page template-place-page membership-${tier.toLowerCase().replace(/\s+/g,'-')}`} style={{'--template-accent':profile.accent} as CSSProperties}>
      ...
      <div className="place-layout site-container">
        <article>
          ...
          {profile.showRelated&&<section className="listing-feature-section listing-related"><span className="site-eyebrow plum">Continue planning</span><h2>Make it part of your trip</h2><div>{guides.slice(0,profile.tier==='Tier 4'?3:2).map((guide)=><article key={guide.slug}><img src={guide.image} alt=""/><span>{guide.eyebrow}</span><h3>{guide.title}</h3><p>{guide.description}</p><button onClick={()=>siteNavigate(`/guide/${guide.slug}`)}>Explore <ArrowRight size={14}/></button></article>)}</div></section>}
        </article>
