import {
  ...
  Heart, LogIn, LogOut, Mail, MapPin, Menu, Search, Sparkles, Star, TrainFront, UserPlus, X,
} from 'lucide-react'
...
const listingTemplateProfiles: ListingTemplateProfile[] = [
  { tier:'Tier 4', listingId:'list-001', templateName:'Signature destination partner', purpose:'A rich, conversion-led page for major attractions and strategic partners.', publicBadge:'Signature partner', accent:'#6d294f', media:'Up to 30 images and 5 videos', includes:['Editorial hero and partner marker','Visitor-first introduction and complete taxonomy','Review-led reasons to visit','Planning summary, practical details and facilities','Prominent direct booking and save actions'], goodForLimit:8, showReviews:true, showPlanningSummary:true, action:'book' },
  { tier:'Tier 3', listingId:'list-005', templateName:'Featured experience', purpose:'An enhanced page for bookable experiences with strong discovery and campaign value.', publicBadge:'Featured member', accent:'#f0785e', media:'Up to 20 images and 3 videos', includes:['Large hero and featured marker','Visitor-first introduction and priority taxonomy','Review themes and visit advice','Full practical information','Direct booking and save actions'], goodForLimit:6, showReviews:true, showPlanningSummary:true, action:'book' },
  { tier:'Tier 2', listingId:'list-007', templateName:'Enhanced member listing', purpose:'A complete, attractive page that helps visitors decide and act with confidence.', accent:'#7a9a83', media:'Up to 12 images and 1 video', includes:['Large hero image','Visitor introduction and selected taxonomy','Three review themes','Opening, access and facility details','Booking or enquiry and save actions'], goodForLimit:5, showReviews:true, showPlanningSummary:false, action:'enquire' },
  { tier:'Tier 1', listingId:'list-009', templateName:'Core member listing', purpose:'A concise member page with the information visitors need to choose and plan.', accent:'#a86b78', media:'Up to 6 images', includes:['Hero image','Short visitor introduction','Four priority taxonomy tags','Opening, access and facility details','Website, enquiry and save actions'], goodForLimit:4, showReviews:false, showPlanningSummary:false, action:'enquire' },
  { tier:'Supplier', listingId:'list-013', templateName:'Visitor economy supplier', purpose:'A business-to-business page focused on services, coverage and qualified enquiries.', accent:'#247a77', media:'Up to 8 images and 1 video', includes:['Service-led hero and introduction','Customer and service taxonomy','Credibility themes from feedback','Coverage, access and operating details','Quote enquiry and website actions'], goodForLimit:5, showReviews:true, showPlanningSummary:false, action:'quote' },
  { tier:'Free Listing', listingId:'list-011', templateName:'Essential directory entry', purpose:'A reliable basic record that keeps eligible places discoverable and accurate.', accent:'#7b8798', media:'1 image', includes:['Single image','Short description','Three priority taxonomy tags','Opening, access and location essentials','Website link when supplied'], goodForLimit:3, showReviews:false, showPlanningSummary:false, action:'website' },
]

function ListingTemplatesPage({ listings, actions }: { listings: Listing[]; actions: VisitorActions }) {
  return <PublicShell savedCount={actions.savedIds.length}><main><PageIntro eyebrow="For review" title="Member listing templates." description="Six proposed page standards, each matched to the visibility and content allowance of its membership level. Review the live examples before the format is applied to other listings." image={imageLibrary.castle}/><section className="template-review site-container"><header><span className="site-eyebrow plum">Approval set</span><h2>One working example for every membership type</h2><p>All templates share accurate visitor information, useful search taxonomy and clear calls to action. Higher tiers add richer storytelling, media, review insight and conversion features.</p></header><div className="template-review-grid">{listingTemplateProfiles.map((profile)=>{const listing=listings.find((item)=>item.id===profile.listingId);return <article key={profile.tier} style={{'--template-accent':profile.accent} as CSSProperties}><div className="template-review-tier"><span>{profile.tier}</span><small>{profile.media}</small></div><h3>{profile.templateName}</h3><p>{profile.purpose}</p><h4>Template includes</h4><ul>{profile.includes.map((item)=><li key={item}><Check size={14}/>{item}</li>)}</ul>{listing&&<button onClick={()=>siteNavigate(`/place/${listing.id}`)}>Review {listing.name} <ArrowRight size={15}/></button>}</article>})}</div><aside><strong>Approval scope</strong><p>These six example pages are the only listings using the proposed tier templates. Approval will provide the basis for updating the remaining member listings and the corresponding CRM content fields.</p></aside></section></main></PublicShell>
}
...
function ListingPage({ listing, actions }: { listing: Listing; actions: VisitorActions }) {
  ...
  const saved = actions.savedIds.includes(listing.id)
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); recordSubmission('vv-listing-enquiries',{ listingId: listing.id, listingName: listing.name, ...values }); setSent(true) }
  const actionLabel = profile.action==='quote'?'Request a quote':profile.action==='enquire'?'Make an enquiry':'Check availability'
  const sectionTitle = profile.action==='quote'?'Services for your business':'Your visit'
  return <PublicShell savedCount={actions.savedIds.length}><main className={`place-page template-place-page membership-${tier.toLowerCase().replace(/\s+/g,'-')}`} style={{'--template-accent':profile.accent} as CSSProperties}><div className="place-breadcrumb site-container"><button onClick={() => siteNavigate('/')}>Home</button><span>/</span><button onClick={() => siteNavigate('/#discover')}>{categoryGroup(listing)}</button><span>/</span><strong>{listing.name}</strong></div><section className="place-hero"><img src={image} alt={listing.name} /><div className="place-hero-copy site-container"><span className="site-eyebrow">{listing.category}</span>{profile.publicBadge&&<span className="place-partner-badge">{profile.publicBadge}</span>}<h1>{listing.name}</h1><p><MapPin size={16} />{listing.town}</p></div></section>{profile.showPlanningSummary&&<div className="place-planning-summary"><div className="site-container"><span><strong>Best for</strong><small>{listing.searchTags.slice(0,2).join(' · ')}</small></span><span><strong>Allow</strong><small>{tier==='Tier 4'?'Half a day or more':'Around two hours'}</small></span><span><strong>Plan ahead</strong><small>{listing.goodToKnow[0]}</small></span></div></div>}<div className="place-layout site-container"><article><p className="place-lede">{listing.shortDescription}</p><div className="place-good-for"><h2>{profile.action==='quote'?'Services and strengths':'Good for'}</h2><div>{listing.searchTags.slice(0,profile.goodForLimit).map((item) => <span key={item}><Check size={13} />{item}</span>)}</div></div><h2>{sectionTitle}</h2><p>{listing.description}</p>{profile.showReviews&&<div className="place-highlights"><h3>{profile.action==='quote'?'Clients frequently mention':'Visitors frequently mention'}</h3><div>{listing.reviewHighlights.map((item) => <span key={item}><Star size={14} />{item}</span>)}</div><small>Themes supported by recent feedback.</small></div>}<h2>{profile.action==='quote'?'Service details':'Good to know'}</h2><dl>{listing.goodToKnow.map((item) => <div key={item}><dt><Check size={14} /></dt><dd>{item}</dd></div>)}<div><dt>{profile.action==='quote'?'Availability':'Opening times'}</dt><dd>{listing.openingHours}</dd></div><div><dt>{profile.action==='quote'?'Services & access':'Accessibility & facilities'}</dt><dd>{listing.facilities.join(' · ')}</dd></div><div><dt>{profile.action==='quote'?'Coverage':'Location'}</dt><dd>{listing.town}, Valechester</dd></div></dl></article><aside><span className="place-book-label">{profile.action==='quote'?'Business enquiry':'Plan your visit'}</span><h3>{listing.name}</h3>{profile.action==='website'?<><a className="place-book" href={listing.website}>Visit website <ArrowRight size={16}/></a>{listing.phone&&<span className="place-contact">Call {listing.phone}</span>}</>:sent ? <div className="place-enquiry-success" role="status"><Check size={22} /><strong>Enquiry sent</strong><small>The team will reply to the email address you provided.</small><button onClick={() => { setSent(false); setEnquiring(false) }}>Done</button></div> : enquiring ? <form className="place-enquiry" onSubmit={submit}><label>Name<input name="name" required /></label><label>Email<input name="email" type="email" required /></label><label>{profile.action==='quote'?'What do you need?':'Preferred date'}<input name="preferredDate" type={profile.action==='quote'?'text':'date'} required /></label><button>Send enquiry <ArrowRight size={14} /></button><button type="button" onClick={() => setEnquiring(false)}>Cancel</button></form> : <>{profile.action==='book'&&listing.bookingUrl?<a className="place-book" href={listing.bookingUrl}>Book direct <ArrowRight size={16}/></a>:<button className="place-book" onClick={() => setEnquiring(true)}>{actionLabel} <ArrowRight size={16} /></button>}{profile.action==='quote'?<a className="place-save-button" href={listing.website}>Visit website <ArrowRight size={14}/></a>:<button className="place-save-button" onClick={() => actions.toggleSaved(listing)}><Heart size={15} fill={saved ? 'currentColor' : 'none'} />{saved ? 'Saved to your trip' : 'Save for later'}</button>}{(profile.action==='enquire')&&<a className="place-contact" href={listing.website}>Visit official website</a>}{listing.phone && <span className="place-contact">Call {listing.phone}</span>}<div><CalendarDays size={18} /><span><strong>{profile.action==='quote'?'Discuss your requirements':'Plan with confidence'}</strong><small>{listing.openingHours}</small></span></div></>}</aside></div></main></PublicShell>
}
